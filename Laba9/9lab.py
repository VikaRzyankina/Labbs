...
class Program:
    def __init__(self):
        self.window = Tk(); self.window.title('Добрый день, пользователь!')
        window=self.window
    ...
    def start(self):
        self.wind = Tk(); self.wind.title('Регистрация')
        wind=self.wind
        ...
        with open('File.txt', 'r') as f:
            ...
            for i in range(0, len(read), 2):
                if self.login_user.get() == read[i].rstrip('\n') and password_sha == read[i+1].rstrip('\n'):
                    Label(self.wind, font = 14, text='Вы успешно авторизовались').pack()
                    NotFound=False
                    break
        if NotFound == True:
            Program.text_window('Не удалось авторизоваться. Не подходящие логин или пароль.')
        else:
            Button(self.wind, font=14, text="Выход", command=self.wind.destroy).pack(anchor=SE)
    ...
    def write_txt(self):
        ...
        with open('File.txt', 'a') as f:
            Label(self.wind, font = 14, text='Вы успешно зарегистрировались.').pack()
